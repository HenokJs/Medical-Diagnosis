...
from backend.services.severity_service import severity_service
...
class DiagnosisController:
    def predict_diagnosis(self, data: Dict) -> Dict:
        ...
        patient_info_payload = data.get('patient_info') or {}

        age = data.get('age', patient_info_payload.get('age'))
        gender = data.get('gender', patient_info_payload.get('gender'))
        duration_days = data.get('duration_days', patient_info_payload.get('duration_days'))
        patient_id = data.get('patient_id', patient_info_payload.get('patient_id'))
        notes = data.get('notes', patient_info_payload.get('notes'))
        ...
        response = {
            ...
            "submitted_symptoms": structured_symptoms,
        ...
        try:
            ...
            session_id = database_service.save_diagnosis_session(response, patient_info)
            if session_id:
                response['session_id'] = session_id
                logger.info("Diagnosis saved to database: %s", session_id)
