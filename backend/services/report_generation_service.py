...
class ReportGenerationService:
    ...
    def generate_clinical_report(self, diagnosis_data: Dict, patient_info: Dict) -> Dict:
        ...
        report = {
            ...
            # Risk Modifiers
            'risk_modifiers': self._build_risk_modifiers_section(diagnosis_data, patient_info),
    ...
    def _build_patient_section(self, patient_info: Dict) -> Dict:
        ...
        symptom_duration = patient_info.get('symptom_duration') or patient_info.get('symptomDuration')
        return {
    ...
    def _build_symptoms_section(self, diagnosis_data: Dict) -> Dict:
        ...
        return {
            ...
            'total_symptoms_processed': diagnosis_data['patient_analysis']['symptoms_processed'],
            'symptoms_matched_in_model': diagnosis_data['patient_analysis']['symptoms_matched']
        }
    
    def _build_analysis_section(self, diagnosis_data: Dict) -> Dict:
        """Build AI diagnostic analysis section."""
        predictions = diagnosis_data['top_predictions'][:3]
    ...
    def _build_reasoning_section(self, diagnosis_data: Dict) -> Dict:
        ...
        return {
            ...
            'influential_symptoms': explainability.get('important_features', [])[:5],
            'confidence_factors': explainability.get('confidence_reasoning', 'Based on symptom pattern analysis'),
            'reasoning_summary': self._generate_reasoning_summary(diagnosis_data)
        }
    
    def _generate_reasoning_summary(self, diagnosis_data: Dict) -> str:
        """Generate a reasoning summary."""
        top_disease = diagnosis_data['top_predictions'][0]['disease']
        confidence = diagnosis_data['top_predictions'][0]['confidence']
        matched = diagnosis_data['patient_analysis']['symptoms_matched']
        
        return (
            f"The AI system identified {top_disease} as the most likely condition "
            f"with {confidence*100:.1f}% confidence. This assessment is based on "
            f"{matched} matched symptoms and clinical pattern recognition."
        )
    
    def _build_severity_section(self, diagnosis_data: Dict) -> Dict:
        """Build severity assessment section."""
        patient_analysis = diagnosis_data['patient_analysis']
        
        severity_descriptions = {
            'minor': 'Minor condition - typically self-limiting and manageable at home',
            'moderate': 'Moderate condition - may require medical consultation',
            'urgent': 'Urgent condition - medical attention recommended soon',
            'critical': 'Critical condition - immediate medical attention required'
        }
        ...
        return {
            ...
            'severity_description': severity_descriptions.get(severity, 'Severity assessment unavailable'),
            'risk_level': patient_analysis.get('risk_level', 'unknown'),
            'urgency_recommendation': self._get_urgency_recommendation(severity)
        }
    
    def _get_urgency_recommendation(self, severity: str) -> str:
        """Get urgency recommendation based on severity."""
        recommendations = {
            'minor': 'Monitor symptoms. Consult healthcare provider if symptoms worsen or persist.',
            'moderate': 'Schedule an appointment with your healthcare provider within 1-2 days.',
            'urgent': 'Seek medical attention within 24 hours or visit urgent care.',
            'critical': 'Seek immediate emergency medical attention. Call emergency services if needed.'
        }
        return recommendations.get(severity, 'Consult with a healthcare professional for guidance.')
    
    def _build_risk_modifiers_section(self, diagnosis_data: Dict, patient_info: Dict) -> Dict:
        ...
        if age < 5:
            modifiers.append({
                ...
                'impact': 'Increased monitoring recommended for young children'
            })
        elif age > 65:
            modifiers.append({
                'type': 'age',
                'factor': 'Elderly patient',
                'impact': 'Higher risk for complications; closer monitoring advised'
            })
        
        # Gender-based modifiers (if relevant)
        if gender == 'female':
            modifiers.append({
                'type': 'gender',
                'factor': 'Female patient',
                'impact': 'Consider pregnancy status and gender-specific conditions'
            })
        
        return {
            'modifiers_applied': len(modifiers) > 0,
            'risk_factors': modifiers,
            'summary': f"{len(modifiers)} risk modifier(s) considered in assessment"
        }
    
    def _build_treatment_section(self, diagnosis_data: Dict) -> Dict:
        """Build treatment guidance section (NON-PRESCRIPTIVE ONLY)."""
        severity = diagnosis_data['patient_analysis'].get('severity', 'unknown')
        ...
        severity_specific = {
            'minor': [
                ...
                'Over-the-counter remedies may provide relief (consult pharmacist)',
        ...
        return {
            ...
            'severity_specific_guidance': severity_specific.get(severity, []),
            'important_notes': [
    ...
    def generate_printable_html(self, report_data: Dict) -> str:
        ...
        Returns:
            HTML string
        """
        # This would generate a professional HTML template
        # For now, return a placeholder
        return f"<html><body><h1>Clinical Report {report_data['report_id']}</h1></body></html>"
